...
import {
  ...
  validateMessages,
  getDifficultyTypes,
  getAllVideos, normFile, uploadFileList2Url, url2UploadFileList, getSpecializedCourseTypes, getAllCoaches,
} from '@/utils/utils';
import TextArea from 'antd/es/input/TextArea';
import { MinusCircleOutlined, PlusCircleOutlined, PlusOutlined, UploadOutlined } from '@ant-design/icons';


const url= urlConfig.SPECIALIZED_COURSE_URL; // modify


const LiveTable: React.FC = () =>{ // modify
  const [data, setData] = useState([]);
  const [difficultyTypes, setDifficultyTypes] = useState([]);
  const [specializedCourseTypes, setSpecializedCourseTypes] = useState([]);
  ...
  useEffect(() => {
    ...
    getSpecializedCourseTypes().then((res: any)=>{
      setSpecializedCourseTypes(res);
    });
    ...
    <PageContainer>
      <Card>
        ...
        <Form form={searchForm} layout="inline">
          ...
          <Form.Item name="type" label="专业课程类型" >
            <Select placeholder="选择类型" allowClear>
              {specializedCourseTypes.map(d => <Option key={d} value={d}>{d}</Option>)}
            </Select>
          </Form.Item>
      ...
      <Card>
        <Table
          columns={ [
            ...
            {
              title: '专业课程课名称', // modify
              dataIndex: 'name',
              key: 'name',
            },
            {
              title: '专业课程课类型', // modify
              dataIndex: 'type',
              key: 'type',
            },
            {
      ...
      >
        <Form
          ...
          onFinish={onAddOk}
        >
          <Form.Item name = "name" label="专业课程名称" rules={[{required: true,},]}>
            <Input />
          </Form.Item>
          ...
          <Form.Item name="type" label="专业课程类型" rules={[{required: true,},]}>
            <Select placeholder="选择类型" allowClear>
              {specializedCourseTypes.map(d => <Option key={d} value={d}>{d}</Option>)}
            </Select>
          </Form.Item>
          <Form.Item name="videoId" label="视频名称" rules={[{required: true,},]}>
            <Select onChange={(videoId)=>{
              addForm.setFieldsValue({duration: videos.filter((v: any)=>v.id===videoId)[0].duration})
            }}>
