...
import { Table, Button, Space, Input, Modal, Form, message, Popconfirm, Card, Upload, Select, InputNumber } from 'antd';
...
import {
  ...
  formLayout,
  validateMessages,
  getAudioTypes, formatFileSize,
} from '@/utils/utils';

const url= urlConfig.AUDIO_URL; // modify

const AudioTable: React.FC = () =>{ // modify
  ...
  const onModifyFormUpload  = () => {
    ...
    if(fileList&&fileList[0].response&&fileList[0].id!=='-1'&&fileList[0].response.code==='000'){
      ...
      video.addEventListener('loadedmetadata', () => {
        if(video.duration===Infinity){
          editForm.setFieldsValue({duration:"时长解析失败，请自行填写！"})
        }else{
    ...
    <PageContainer>
      <Card>
        ...
        <Form form={searchForm} layout="inline">
          ...
          <Form.Item name="type" label="音频类型">
            <Select placeholder="音频类型" allowClear >
              {audioTypes.map(d => <Option key={d} value={d}>{d}</Option>)}
            </Select>
          </Form.Item>
          <Form.Item name="name" label="音频名称">
            <Input/>
          </Form.Item>
          <Form.Item>
      ...
      >
        <Form
          ...
          onFinish={onAddOk}
        >
          <Form.Item name = "name" label="音频名称" rules={[{required: true,},]}>
            <Input />
          </Form.Item>
          <Form.Item name="type" label="音频类型" rules={[{required: true,},]}>
            <Select placeholder="音频类型" allowClear >
              {audioTypes.map(d => <Option key={d} value={d}>{d}</Option>)}
            </Select>
          </Form.Item>
          <Form.Item name = "url" label="音频文件" valuePropName="fileList" getValueFromEvent={normFile} rules={[{required: true,},]}>
            <Upload
              name="file"
              listType="picture"
              action={urlConfig.AUDIO_UPLOAD_URL}
