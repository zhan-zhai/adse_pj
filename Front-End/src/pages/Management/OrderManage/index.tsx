...
import {
    ...
    updateOrder,
    payOrder, getOrderBystatus, deliverOrder,
} from '@/utils/utils';
...
const getDeliverCompany_url = urlConfig.DELIVER_COMPANY_GET_URL;
const delivery_url = urlConfig.ORDER_DELIVER_URL;
...
const CoachTable: React.FC = () => { // modify
  ...
  const [modifyDeliverVisible, setModifyDeliverVisible] = useState(false);
  ...
  const getData = async () => {
    ...
    const getDeliverCompany = async () => {
        const res = await getItems(getDeliverCompany_url, "")
        if (res) {
            console.log(res.data);
            setDCompany(res.data);
            console.log("123");
            console.log(data);
  ...
  const onModify = (record: any) => {
    ...
    const onModifyDeliver = (record: any) => {
        console.log(record);
        //每次弹出来将address选择框置为空
        getDeliverCompany();
        console.log(dCompany);
  ...
  const onModifyOk = async () => {
    ...
    const onModifyDeliverOk = async () => {
        const formValues = deliverForm.getFieldsValue();
        console.log("deliverForm",formValues);
        var requestParam;
        requestParam = {
            orderItemId: formValues.orderItemId,
            companyName: formValues.deliverCompany
        }
        console.log("22222",requestParam);
        const success = await deliverOrder(delivery_url,requestParam.orderItemId, requestParam.companyName);
        if (success) {
            setModifyDeliverVisible(false);
            getData();
        }
    };
            ...
            },
            {
              ...
              render: (text: any, record: any) => (
                <Space size="middle">
                    ...
                    <Button type="primary" ghost onClick={() => onModifyDeliver(record)} disabled={(toNumber(record.status) >= 2)}>发货</Button>
...
{/*                      <Link to = "/order/logistic">
      ...
      >
        ...
        <Modal
            title="发货"
            visible={modifyDeliverVisible}
            okButtonProps={{ htmlType: 'submit', form: 'deliverForm' }}
            onCancel={() => {setModifyDeliverVisible(false)}}
        >
            <Form name="deliverForm" form={deliverForm} {...formLayout} onFinish={onModifyDeliverOk} validateMessages={validateMessages}>
                <Form.Item name="orderItemId" label="订单编号" >
                    <Input disabled/>
                </Form.Item>
                <Form.Item name="userName" label="用户" >
                ...
                <Form.Item name="deliverCompany" label="快递商" rules={[{ required: true, },]}>
                  <Select placeholder={"选择发货快递"}>
                      {dCompany.map((d: any) => <Option value={d.name}>{d.name}</Option>)}
                  </Select>
                </Form.Item>
                <Form.Item
                    name="addressDetail"
                ...
                {/*<Text>如果修改的话，不用输入省市区</Text>*/}
                <Form.Item name="address" label="基本地址" >
                    <Cascader name="addCas" options={options.options} placeholder="可修改省/市/区" onChange={onChange} disabled/>
                </Form.Item>
                {/*<Form.Item name="memberShipGoodsType" label="订单会员类型" rules={[{ required: true, },]}>*/}
                {/*  <Select placeholder={"选择订单会员状态"}>*/}
                {/*    <Option value={"MONTH"}>月度会员</Option>*/}
