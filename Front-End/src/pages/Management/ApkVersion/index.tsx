...
const url = urlConfig.APK_VERSION; // modify

const ApkVersionTable: React.FC = () => { // modify
  ...
  const onAddOk = async () => {
    ...
    const apkUrl = uploadFileList2Url(formValues.apkUrl);
    if (apkUrl) {
      const success = await addItem(url, { ...addForm.getFieldsValue(), apkUrl });
      if (success) {
  ...
  const onModify = (record: any) => {
    console.log(record);
    editForm.setFieldsValue({ ...record, apkUrl: url2UploadFileList(record.apkUrl) });
  ...
  const onModifyOk = async () => {
    ...
    const apkUrl = uploadFileList2Url(formValues.apkUrl);
    if (apkUrl) {
      const success = await updateItem(url, editForm.getFieldValue("id"), { ...editForm.getFieldsValue(), apkUrl });
      if (success) {
      ...
      >
        ...
        >
          <Form.Item name="version" label="版本号" rules={[{ required: true, },]}>
            <Input placeholder={"格式x.x.x，例：1.1.12"}/>
          </Form.Item>
          <Form.Item
            name="apkUrl"
          ...
          >
            <Upload
              name="file"
              listType="picture"
              action={urlConfig.APK_UPLOAD_URL}
